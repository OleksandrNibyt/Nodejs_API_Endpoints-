...
app.post('/user/new/', (request, response) => {
  // get name and age from the req body
  const { name, age } = request.body;
  // generate an id
  const id = fake_database.length + 1;
  // create a new user object to store all the info
  const newUser = {
    id,
    name,
    age,
  };
  // add new user to the fake_database
...
app.put('/user/update/name/', (request, response) => {
  // grab id and name from req body
  const { name, id } = request.body;

  // loop on fake_database
